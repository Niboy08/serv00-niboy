...
save_config() {
    local port=$1
    if [[ ! -f ~/domains/bolan.niboy.top/vless/config.json ]]; then
        uuid=$(generate_uuid)
        cat <<EOL > ~/domains/bolan.niboy.top/vless/config.json
{
    "uuid": "$uuid",
    "port": $port
}
EOL
        ...
        jq --arg port "$port" '.port = ($port | tonumber)' ~/domains/bolan.niboy.top/vless/config.json > ~/domains/bolan.niboy.top/vless/config_tmp.json && mv ~/domains/bolan.niboy.top/vless/config_tmp.json ~/domains/bolan.niboy.top/vless/config.json
...
deploy_vless() {
    ...
    npm install
    # 启动vless项目
    ~/.npm-global/bin/pm2 start ~/domains/bolan.niboy.top/vless/app.js --name vless
    # 保存pm2进程状态
    ...
    echo -e "域名: ${GREEN}bolan.niboy.top${NC}"
    echo -e "vless进程维护定时任务脚本: ${GREEN}cd ~/domains/bolan.niboy.top/vless && ./check_vless.sh${NC}"
    echo -e "VLESS节点信息: ${GREEN}vless://${uuid}@bolan.niboy.top:${port}?flow=&security=none&encryption=none&type=ws&host=bolan.niboy.top&path=/&sni=&fp=&pbk=&sid=#$USER.serv00.vless${NC}"
}
...
main() {
    ...
    if [ "$port_provided" = true ]; then
        ...
        if [[ -f config.json ]]; then
            ...
            echo -e "UUID: ${uuid}"
            echo -e "Port: ${port}"
            echo -e "域名: bolan.niboy.top"
            echo -e "VLESS节点信息: vless://${uuid}@bolan.niboy.top:${port}?flow=&security=none&encryption=none&type=ws&host=bolan.niboy.top&path=/&sni=&fp=&pbk=&sid=#$USER.serv00.vless"
